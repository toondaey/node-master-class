...
const __data = require('../data'),
    ...
    { debuglog, promisify } = require('util'),
...
lib.__orders.post = async data => {
    ...
    let number, exp_month, exp_year, cvc, currency, order_id;

    // Extract required data.
    try {
        ({ card: { number, exp_month, exp_year, cvc }, currency, order_id } = data.payload);
    } catch (e) {
        debug(e);
    ...
    try {
        ...
        const resToken = await createPaymentToken({ card: { exp_month, exp_year, number, cvc } });

        // Charge order.
        debug('Creating charge...')
        const resCharge = await createPaymentCharge({ source: resToken.card, amount: order.amount, currency }, order_id);
    } catch (e) {
        // For debugging...
        debug(e);
    ...
    return {
        statusCode: 200,
        responsePayload: { message: 'Thank you for your patronage. Your order is being processed and should be delivered in 30 mintues.' }
    };
};
