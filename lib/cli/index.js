...
const readline = require('readline'),
    ...
    __data = require('../data'),
    { inspect: inspectUtil } = require('util'),
    os = require('os');
 ...
 * @return {undefined}
 */
lib._handlers.user = async email => {
    ...
    if (!(firstName && lastName && emailAddress && address)) return lib._ui.error('User does not exist');

    // Get the width of the screen when command was entered.
...
lib._handlers.users = async ({_prompts, parameters}) => {
    ...
    if ((email = parameters.findIndex(param => /^--email=.+$/.test(param))) !== -1)
        return lib._handlers.user(parameters[email]);
 ...
 * @return {undefined}
 */
lib._handlers.order = async (id, dirs) => {
    ...
    if (!filename) process.stdout.write(lib._ui.error(`Order with id [${id}] does not exist.\n`));
    ...
    // Insert space
    process.stdout.write(`${lib._ui.horizontalFill(width)}\n`);
    process.stdout.write(inspectUtil(order, { colors: true }))
    // Insert space.
    process.stdout.write(`${lib._ui.horizontalFill(width)}\n`);
};
...
lib._handlers.orders = ({ _prompts, parameters }) => {
    ...
    if ((id = parameters.findIndex(param => /^--id=.+$/.test(param))) !== -1)
        return lib._handlers.order(parameters[id], dirs);
    ...
    setTimeout(() => process.stdout.write(`${lib._ui.horizontalFill(width, { fill: '-' })}\n`), 50);
};
...
lib.init = () => {
    ...
    _interface.on('line', input => {
        ...
        else if (!input) lib._ui.error('No command provided.');
