...
lib._handlers.users = async ({_prompts, parameters}) => {
    ...
    users.forEach(async user => {
        if (parameters.indexOf('--latest') !== -1) {
...
lib._handlers.orders = async ({_prompts, parameters}) => {
    // const orders = await __data.list('orders/pending');
...
lib._prompts = {
    ...
    orders: {
        ...
        help: {
            key: 'orders [--latest]',
            message: `Show orders. Use \x1b[1m--latest\x1b[0m flag to get the pending orders within 24 hours.`,
        },
    },
    users: {
